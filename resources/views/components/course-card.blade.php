<a href="{{ route('front.course.details', $course->slug) }}" class="group block transition-all duration-200 hover:-translate-y-1 cursor-pointer">
        ...
        <div class="flex flex-col p-6 pt-2 space-y-4">
            ...
            <div class="pt-2">
                ...
                <div class="flex items-center justify-between text-xs text-gray-500 mt-2 pt-2 border-t border-gray-100">
                    <span>{{ $course->course_students_count ?? ($course->relationLoaded('courseStudents') ? $course->courseStudents->count() : 0) }} students</span>
                    <span>{{ $course->course_sections_count ?? ($course->relationLoaded('courseSections') ? $course->courseSections->count() : 0) }} sections</span>
                </div>
            </div>
        </div>
    </div>
